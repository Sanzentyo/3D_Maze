...
class StartScene(Scene):
    ...
    def draw(self):
        ...
        self.writer.draw(self.center[0]-300+15, self.center[1]-100+15, "3D Maze", 180, pyxel.COLOR_GRAY)
        self.writer.draw(self.center[0]-300, self.center[1]-100, "3D Maze", 180, pyxel.COLOR_WHITE)
        # 軽めの明滅処理を追加
        if pyxel.frame_count % 30 < 27:
            self.writer.draw(self.center[0]-165, pyxel.height-95, "Press Space", 60, pyxel.COLOR_BLACK)

        # BGMの再生を追加
        from bgm_data import start_scene_bgm_data
        self.bgm_data = start_scene_bgm_data
        if pyxel.play_pos(0) is None:
            for ch, sound in enumerate(self.bgm_data):
...
class GameScene(Scene):
    ...
    def __init__(self, global_state):
        ...
        # BGMの再生を追加
        from bgm_data import game_scene_bgm_data
        self.bgm_data = game_scene_bgm_data
        if pyxel.play_pos(0) is None:
            for ch, sound in enumerate(self.bgm_data):
                pyxel.sounds[ch].set(*sound)
                pyxel.play(ch, ch, loop=True) 
...
class ScoreBoard:
    ...
    def __init__(self, elapsed_time:float, screen_width:int, screen_height:int):
        ...
        self.time_str = f"{minutes:02d}:{seconds:02d}.{milliseconds:03d}"
    ...
    def draw(self):
        ...
        pyxel.rect(board_x + 2, board_y + 2, board_width - 4, board_height - 4, pyxel.COLOR_GRAY)

        # テキストの描画
        self.writer.draw(board_x + 20, board_y + 40, "GOAL REACHED!", 85, pyxel.COLOR_YELLOW)
        self.writer.draw(board_x + 70, board_y + 170, f"Time: {self.time_str}", 60, pyxel.COLOR_WHITE)
        self.writer.draw(board_x + 140, board_y + 250, "Press Tab to hide scoreboard", 20, pyxel.COLOR_WHITE)
